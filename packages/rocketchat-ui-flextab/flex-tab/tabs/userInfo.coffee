...
Template.userInfo.helpers
	...
	dist: ->
		thatuser = Template.instance().user.get()
		if thatuser.geo
			me = Meteor.user()
	#		d1 = me.Location
	#		d2 = thatuser.Location
	#		ad1 = d1.split(',')
	#		ad2 = d2.split(',')
			firstLatRad = me.geo.latitude * Math.PI / 180
			firstLongRad = me.geo.longitude * Math.PI / 180
			secondLatRad = thatuser.geo.latitude * Math.PI / 180
			secondLongRad = thatuser.geo.longitude * Math.PI / 180
			radius = 6371
			distance = Math.acos(Math.sin(firstLatRad) * Math.sin(secondLatRad) + Math.cos(firstLatRad) * Math.cos(secondLatRad) * Math.cos(firstLongRad - secondLongRad)) * radius
	...
	gallery: ->
		user = Template.instance().user.get()
		return user.galleryImg

	isGallery: (username) ->
		user = Template.instance().user.get()
		if user.galleryImg && user.username isnt username
			return false
		else
			return true
	...
	width: (username) ->
		user = Template.instance().user.get()
		if Meteor.user()?.username isnt username && user.galleryImg
			return parseInt(user.galleryImg.length) * 154
		if user.galleryImg
			return (parseInt(user.galleryImg.length) + 1) * 154
	...
	Orientation: ->
		user = Template.instance().user.get()
		return TAPi18n.__ user.orientation
	...
	Homecity: ->
		user = Template.instance().user.get()
		return user.homecity

	Sex: ->
		user = Template.instance().user.get()
		return TAPi18n.__ user.sex
	...
	Restdate: ->
		user = Template.instance().user.get()
		return moment(user.restdate).format('LL')

	Birthday: ->
		user = Template.instance().user.get()
		return moment(user.birthday).format('LL')
...
Template.userInfo.events
	...
	'change #gal-upload': (event, template) ->
		...
		for file in files
			filesToUpload.push
				file: file
				name: file.name
		userGalleryImgUpload filesToUpload



	'click .customsave': (e, instance) ->
	...
	'click .like-user': (e, instance) ->
		e.stopPropagation()
		e.preventDefault()
		console.log instance.user.get()
		Meteor.call 'likeUser', { _id: instance.user.get()._id, username: instance.user.get().username }, (error, result) ->
			if error
				handleError(error)
...
Template.userOrg.onCreated ->
	@subscribe 'orgById', Template.instance().data.org


Template.userOrg.helpers
	data: ->
		orgdata = Orgs.findOne(Template.instance().data.org)
		return orgdata
