...
export class CustomOAuth {
	...
	getAccessToken(query) {
		...
		if (response.data.error) { //if the http response was a json object with an error attribute
			...
			//response.data.client_secret = OAuth.openSecret(config.secret);
			console.log('----> ' + query.code);
			console.log(response.data);
			return response.data.access_token;

			//return 'a7c53470a7c53470a77716e869a79cc326aa7c5a7c53470ff7795ada2f28d7253fa15fd';

		}
	}

	getIdentity(accessToken, query) {
		const config = ServiceConfiguration.configurations.findOne({service: this.name});
		const params = {};
		const headers = {
		...
		if (this.tokenSentVia === 'header') {
			...
			headers['client_secret'] = OAuth.openSecret(config.secret);
			...
			//if(this.name === 'vk2'){
			//	params['fields'] = 'photo_400_orig,city,nickname,sex,bdate,city,home_town,interests';
			//}
	...
	registerService() {
		...
		OAuth.registerService(this.name, 2, null, (query) => {
			...
			let identity = self.getIdentity(accessToken, query);
			...
			if (identity) {
				...
				//console.log('ident.res[0]');
				//console.log(identity.response[0]);
				//console.log('ident.res[0].id');
				//console.log(identity.response[0].id);
				////console.log('json parse');
				////obj = JSON.parse(identity.response[0]);
				////console.log(obj);
				////console.log(obj.id);
				...
				// Fix general 'userid' instead of 'id' from provider
				if (!identity.id && identity.response ) {
					identity.id = translit(identity.response[0].first_name +'-'+ identity.response[0].last_name);
...
Accounts.updateOrCreateUserFromExternalService = function(serviceName, serviceData, options) {
	...
	console.log('ошибка в самом низу');
