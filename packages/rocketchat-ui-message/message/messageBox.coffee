	...
	'click .message-form .message-buttons.location': (event, instance) ->
		...
		text = """
					<div class='location-preview'>
						<div class="location-preview">
							<img style="height: 250px; width: 250px;" src="https://maps.googleapis.com/maps/api/staticmap?zoom=14&language=ru&size=250x250&markers=color:gray%7Clabel:%7C#{latitude},#{longitude}&key=#{RocketChat.settings.get('MapView_GMapsAPIKey')}" />
						</div>
						<input id='geo-description' style='display: inherit;' value='' placeholder='#{t("Message")}'>
					</div>
				"""
#		"""
#			<div class="location-preview">
...
#		"""
		...
		, (isConfirm) ->
			...
			Meteor.call "sendMessage",
				_id: Random.id()
				rid: roomId
				msg: document.getElementById('geo-description').value
				location:
					type: 'Point'
...
##				Выставляем город пользователя при первом входе в систему
#	if !Meteor.user().city
#		geo = new GeoCoder(
#			geocoderProvider: 'yandex'
#			httpAdapter: 'https'
#			lang: 'ru_RU')
#		geodata = geo.reverse(Meteor.user().geo.latitude, Meteor.user().geo.longitude)
#		city = geodata[0].city
#		Meteor.call 'setCity', city, (err,res) ->
#			if err
#				console.log err
#			if res
#				console.log res
#				Session.set 'current-city', res
	...
	Tracker.autorun ->
		if RocketChat.settings.get('MapView_Enabled') is true and RocketChat.settings.get('MapView_GMapsAPIKey')?.length and navigator.geolocation?.getCurrentPosition?
			success = (position) =>
				...
				if flag is false
#					swal
#						title: t('Поделиться местоположением?')
#						showCancelButton: true
#						closeOnConfirm: true
#						closeOnCancel: true
#						html: true
...
#							return
					...
					Meteor.call 'setUserGeo', geo, (error, result) ->
...
#
#
#	if RocketChat.Geolocation.get() isnt false
#		swal
#			title: t('Поделиться местоположением?')
#			showCancelButton: true
#			closeOnConfirm: true
...
#			user = Meteor.userId()
#			Meteor.call 'setLocation', { id: user, string: str }, (error, result) ->
