...
#		name = instance.find('#channel-name').value.toLowerCase().trim()
		...
		runame = instance.find('#channel-name').value.trim()
		name = translit(runame.toLowerCase().trim())
		...
		if not err
			Meteor.call createRoute, name, instance.selectedUsers.get(), readOnly, fields, (err, result) ->
				if err
					console.log err
	...
	'click .addorgbutton': ->
		FlowRouter.go 'add-new-org-front'
#		SideNav.closeFlex ->
#			instance.clearForm()
#		menu.close()
