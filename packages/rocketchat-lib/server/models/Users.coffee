class ModelUsers extends RocketChat.models._Base
	...
	findMoodByUsername: (username) ->
		query =
			_username: username

		return @find(query)?.fetch?()?[0]?.mood


	findBySex: (sex, options) ->
		query =
			sex:
				$in: types
	...
	findBySexAndName: (filter, sex, options) ->
	...
	setOrientation: (_id, orientation) ->
		update =
			$set: orientation: orientation

		return @update _id, update

	setDescription: (_id, description) ->
		update =
	...
	triggerRuletka: (_id, ruletka) ->
		update =
			$set: ruletka: ruletka

		return @update _id, update

	setInteres: (_id, interes) ->
		update =
			$set: interes: interes
	...
	setBirthday: (_id, birthday) ->
		update =
			$set: birthday: birthday

		return @update _id, update

	setRestDate: (_id, restdate) ->
		update =
			$set: restdate: restdate
	...
	setGalleryImg: (_id, fileId, fileName) ->
		query =
			_id: _id
		update =
			$addToSet:
				galleryImg: {id: fileId, name: fileName}
