...
RocketChat.roomTypes.add('c', 10, {
	...
	findRoom(identifier) {
		...
		console.log(ChatRoom.findOne(query));
...
RocketChat.roomTypes.add('d', 20, {
	...
	getUserMood(roomId) {
		...
		return RocketChat.models.Users.find({username: subscription.name}).fetch()[0].mood;
		//return Session.get(`user_${subscription.name}_mood`);
	}
});
